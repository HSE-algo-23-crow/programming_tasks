...
def get_invest_distribution(profit_matrix: list[list[int]]) -> \
    ...
    distributions = __get_distribution(max_profit_matrix, profit_matrix)

    return {PROFIT: profit, DISTRIBUTION: distributions}
...
def __get_distribution(max_profit_matrix, profit_matrix):
    ...
    for proj_idx in range(ex_proj_cnt - 1, 0, -1):
        ...
        for prev_level in range(target_level + 1):
            ...
            cur_profit = 0 if cur_level == 0 else profit_matrix[cur_level-1][proj_idx]
